                    ...
                    await using (var connection = new MySqlConnection(dbConnectionString))
                    {
                        ...
                        if (await reader.ReadAsync() == false)
                        {
                                ...
                                if (await reader2.ReadAsync())
                                {
                                    ...
                                    if (await reader3.ReadAsync())
                                    {
                                        ...
                                        Server.NextFrame(() =>
                                        {
                                            ...
                                            Server.PrintToChatAll($" {Localizer["Prefix"]} {Localizer["chat<create_success>", playerName, message]}");
                                        });
                                    }
                                    reader3.Close();
                                }
                            ...
                            else if(userInfo[slot].Status == 2)
                            {
                                ...
                                {
                                    if(StoreApi != null) StoreApi.GivePlayerCredits(player, -Config.RenameCost.Value);
                                }
                                Server.NextFrame(() =>
                                {
                                    Server.PrintToChatAll($" {Localizer["Prefix"]} {Localizer["chat<rename_success>", playerName, message]}");
                                });
                            }
                        }
                        else
                        {
                            Server.NextFrame(() => player.PrintToChat(Localizer["chat<rename_exist>"]));
                        }
                    }
                }
                catch (Exception ex)
