...
{
    private static Gangs Instance = Gangs.Instance;
    ...
    static public void Open(CCSPlayerController? player)
    {
        ...
        var menu = new ChatMenu(Instance.Localizer["menu<title>"]);
        ...
        if (Instance.userInfo[slot].DatabaseID != -1)
        {
            ...
            if (gang != null)
            {
                ...
                {
                    var skillsMenu = new ChatMenu(Instance.Localizer["menu<skills>"]);
                ...
                {
                    var acceptMenu = new ChatMenu(Instance.Localizer["menu<leave_accept>"]);
    ...
    static public void OpenAdminMenu(CCSPlayerController? player)
    {
        ...
        var menu = new ChatMenu(Instance.Localizer["menu<title_with_name>", gang.Name]);

        var sizeSkill = gang.SkillList.Find(x => x.Name.Equals("size"));

        menu.AddMenuOption(Instance.Localizer["menu<invite>"], (player, option) =>
        {
            var usersMenu = new ChatMenu(Instance.Localizer["menu<players>"]);
            ...
            foreach (var user in Utilities.GetPlayers())
            {
                ...
                if (Instance.userInfo[user.Slot].DatabaseID == -1)
                {
                    usersMenu.AddMenuOption($"{user.PlayerName}", (inviter, option) =>
                    {
                        ...
                        var acceptMenu = new ChatMenu(Instance.Localizer["menu<invite_came>", gang.Name]);
        ...
        if (Instance.Config.ExtendCost.Value.Count > 0 && Instance.StoreApi != null)
        {
            ...
            {
                var pricesMenu = new ChatMenu(Instance.Localizer["menu<extend_date>"]);
                ...
                try
                {
                    await using (var connection = new MySqlConnection(Instance.dbConnectionString))
                    {
                        ...
                        foreach (var user in users)
                        {
                            ...
                            {
                                var acceptMenu = new ChatMenu(Instance.Localizer["menu<kick_sure>", user.Value]);
                                acceptMenu.AddMenuOption(Instance.Localizer["Yes"], (player, option) =>
                                {
                                    Task.Run(async () =>
                                    {
                ...
                try
                {
                    await using (var connection = new MySqlConnection(Instance.dbConnectionString))
                    {
                        ...
                        foreach (var user in users)
                        {
                            usersMenu.AddMenuOption($"{user.Value}", (player, option) =>
                            {
                                var acceptMenu = new ChatMenu(Instance.Localizer["menu<leader_sure>", user.Value]);
                                acceptMenu.AddMenuOption(Instance.Localizer["Yes"], (player, option) =>
                                {
                                    Task.Run(async () =>
                                    {
        ...
        if (Instance.userInfo[slot].Rank == 0)
        {
            ...
            {
                var confirmMenu = new ChatMenu(Instance.Localizer["Sure"]);
        ...
        Task.Run(async () =>
        {
            try
            {
                await using (var connection = new MySqlConnection(Instance.dbConnectionString))
                {
                    ...
                    Server.NextFrame(() => {
                        var statmenu = new ChatMenu(Instance.Localizer["menu<statistic_title>"]);
